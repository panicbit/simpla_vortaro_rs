...
async fn sercxi(vorto: &str, width: usize) -> sv::Result<()> {
    ...
    if !trovo.vortfarado.is_empty() {
        ...
        for vortfarado in &trovo.vortfarado {
            ...
            let mut partoj = vortfarado
                .partoj
                .iter()
                .flat_map(|p| &p.vorto)
                .map(|v| v.bold().bright_green());
...
fn print_translations(tradukoj: &[sv::Traduko], indent: usize) {
    if !tradukoj.is_empty() {
        ...
        for (lingvo, traduko) in &tradukoj {
            let mut vortoj = traduko.map(|tra| {
                if let Some(ref vorto) = tra.vorto {
                    format!(
                        "{} ({})",
                        vorto.bold().bright_green(),
                        tra.traduko.bold().magenta()
                    )
                } else {
